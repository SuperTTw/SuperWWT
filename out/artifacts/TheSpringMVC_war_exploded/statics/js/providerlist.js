...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		$.ajax({
			type:"GET",
			url:path+"/provider/providerview",
			...
			success:function(data){
				if ("failet"==data){
					...
					$("#p_proCode").val(data.proCode);
					$("#p_proName").val(data.proName);
					$("#p_proContact").val(data.proContact);
					$("#p_proPhone").val(data.proPhone);
					$("#p_proFax").val(data.proFax);
					$("#p_proDesc").val(data.proDesc);
				}
			},
			error:function(data){
				alert("error!")
	...
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/upd/"+ obj.attr("proid");
	});
